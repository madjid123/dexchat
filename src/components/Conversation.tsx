...
import { MessagesSelector, Message, addMessage, clearAllMessages, setMessagesState, SendMessageToApi, LoadMessages } from "../features/Conversation/MessagesSlice"
import { useAppDispatch } from "../app/hooks"
import { MessageEndPointApi, useGetMessagesByRoomIdQuery, useLazyGetMessagesByRoomIdQuery } from "../services/MessageApi"
...
function Conversation(props: ConversationProps) {
    ...
    const [trigger, result, LastPromiseIngo] = MessageEndPointApi
        .endpoints
        .getMessagesByRoomId
        .useLazyQuery({
    ...
    const onMessage = () => {
        ...
        if (socket.connected && currentUser !== undefined) {
            ...
            dispatch(addMessage(_message))
            dispatch(SendMessageToApi({ message: _message, room_id: props.CurrentRoomId }))
            setMessage("")
        }
    }
    ...
    useEffect(() => {
        if (currentUser !== undefined) {
            ...
            dispatch(MessageEndPointApi.endpoints.getMessagesByRoomId.initiate({ room_id: props.CurrentRoomId, page: 1 }, { forceRefetch: true }))
    ...
    const fetchMessages = () => setTimeout(() => {
        ...
        console.log({ room_id: props.CurrentRoomId, page: messagesResponse.page + 1 })
            ...
            <div className='conversation' onKeyPress={(e) => {
                ...
                >
                    ...
                    <InfiniteScroll
                        ...
                        next={fetchMessages}
                        hasMore={(messagesResponse.page < messagesResponse.pages) ? true : false}
                        loader={<div style={{ alignItems: "center", textAlign: "center", justifyContent: 'center' }}><Spinner animation="grow" variant="light" /></div>}
                        endMessage={
                            <div> <hr></hr></div>
                        }
                        ...
                        inverse={true}
                        onScroll={() => { console.log("Wow Scrolling!") }}
                    //height="auto"
                    >
                        <ListGroup >
                            {
                                messagesResponse.messages.map((msg: Message, index) => {
                                    return (
                                        <ListGroup.Item variant="dark" key={index} className="MessageItem" style={{ backgroundColor: "inherit" }} >
                                            <div className="message">
                                                <div className={(msg.Sender.username === currentUser?.name) ? "my-message-bull" : "other-message-bull"}>
                                                    <div></div>
